...
class _ArenaScreenState extends State<ArenaScreen> {
  ...
  final String _playerId = FirebaseAuth.instance.currentUser?.uid ?? DateTime.now().millisecondsSinceEpoch.toString();
  final String _playerName = FirebaseAuth.instance.currentUser?.displayName ?? 'Aspirante';
  ...
  void _handleMatchFinished() {
    ...
    final myScore = isPlayer1 ? (_match?.player1Score ?? 0) : (_match?.player2Score ?? 0);
    final oppScore = isPlayer1 ? (_match?.player2Score ?? 0) : (_match?.player1Score ?? 0);
  ...
  void _answer(int selectedIndex) {
    ...
    setState(() { _answeredCurrent = true; });

    final q = _match!.questions[_currentQuestionIndex];
    if (selectedIndex == q['correctAnswer']) {
      final isPlayer1 = _match!.player1Id == _playerId;
      final currentScore = isPlayer1 ? _match!.player1Score : _match!.player2Score;
      _arenaService.updateScore(_match!.id, isPlayer1 ? 'player1Score' : 'player2Score', currentScore + 10);
    }
  ...
  Widget build(BuildContext context) {
    return Scaffold(
      ...
      appBar: AppBar(
        ...
        elevation: 0,
        title: const Text('Arena Policial', style: TextStyle(color: Colors.white)),
        leading: IconButton(
      ...
      body: SafeArea(
        child: _match == null ? _buildLobby() : _buildGameArea(),
      ),
    );
  }
    ...
    return Center(
      child: Column(
        ...
        children: [
          ...
          const Text('Demuestra tus conocimientos contra otro aspirante.', style: TextStyle(color: Colors.white70)),
          ...
          if (_isSearching)
            ...
            ElevatedButton(
              ...
              child: const Text('BUSCAR OPONENTE', style: TextStyle(fontWeight: FontWeight.bold)),
            ),
        ],
      ),
    );
  ...
  Widget _buildGameArea() {
    ...
    final isPlayer1 = _match!.player1Id == _playerId;
    final myName = isPlayer1 ? _match!.player1Name : (_match!.player2Name ?? 'Rival');
    final oppName = isPlayer1 ? (_match!.player2Name ?? 'Rival') : _match!.player1Name;
  ...
  Widget _buildPlayerScore(String name, int score, Color color) {
    return Column(
      children: [
        Text(name, style: const TextStyle(color: Colors.white70, fontWeight: FontWeight.bold)),
  ...
  Widget _buildResults(String myName, int myScore, String oppName, int oppScore) {
    ...
    return Center(
      child: Column(
        mainAxisAlignment: MainAxisAlignment.center,
        children: [
          Icon(won ? Icons.emoji_events : (tie ? Icons.handshake : Icons.sentiment_dissatisfied), 
               size: 80, color: won ? Colors.amber : Colors.white54),
          const SizedBox(height: 20),
          Text(won ? '¡VICTORIA!' : (tie ? 'EMPATE' : 'DERROTA'), 
               style: TextStyle(color: won ? Colors.amber : Colors.white, fontSize: 32, fontWeight: FontWeight.bold)),
          const SizedBox(height: 30),
          Text('$myName: $myScore', style: const TextStyle(color: Colors.white, fontSize: 20)),
          const SizedBox(height: 10),
          Text('$oppName: $oppScore', style: const TextStyle(color: Colors.white54, fontSize: 18)),
          const SizedBox(height: 40),
          if (won) const Text('+50 XP', style: TextStyle(color: Colors.greenAccent, fontSize: 24, fontWeight: FontWeight.bold)),
          const SizedBox(height: 40),
          ElevatedButton(
            style: ElevatedButton.styleFrom(
              backgroundColor: NeuralDesignSystem.accentPurple,
