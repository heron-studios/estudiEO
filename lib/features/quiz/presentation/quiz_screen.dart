...
class _QuizScreenState extends State<QuizScreen> {
  ...
  Future<void> _askTutor(Question question, int selected) async {
    ...
    final respElegida = selected >= 0 && selected < question.options.length
    ...
    final explanation = await GeminiService.explicarError(
      question.text,
      respElegida,
      respCorrecta,
    );
        ...
        builder: (ctx) => AlertDialog(
          ...
          actions: [
            TextButton(
              ...
              child: const Text(
                'Entendido',
                style: TextStyle(color: NeuralDesignSystem.blueGoogle),
              ),
            ),
          ],
        ),
                          ...
                          child: Column(
                            ...
                            children: [
                              ...
                              ...List.generate(question.options.length, (i) {
                                Color bg = NeuralDesignSystem.surfaceCard
                                    .withValues(alpha: 0.6);
                                Color border = Colors.white.withValues(
                                  alpha: 0.1,
                                );
                                Color textColor =
                                    NeuralDesignSystem.textPrimaryAlt;
                                Widget? trailing;
                                ...
                                if (isAnswered) {
                                  ...
                                  bg = NeuralDesignSystem.blueGoogle.withValues(
                                    alpha: 0.12,
                                  );
                              ...
                              if (isAnswered &&
                                ...
                                Center(
                                  child: OutlinedButton.icon(
                                    onPressed: () =>
                                        _askTutor(question, _selectedAnswer),
                                    icon: const Text(
                                      '🤖',
                                      style: TextStyle(fontSize: 18),
                                    ),
                                    label: const Text(
                                      'Preguntar al Tutor IA por qué fallé',
                                    ),
